...
import { GuestDb, ShopItemFoodEnums, ShopItemFoodEnumMap, ShopItemDrinkEnums } from "./globals";
import { isValidGuest, setGuestDestination, getGuestsOnNeighbouringTile } from "./util";

const log = new Logger("stalls", 1);
...
export class StallPingScheduler {
    ...
    constructor(guestDb: GuestDb, pingInterval: number) {
        // this object doesn't really own the guestDb
        this.db = guestDb;
        this.pingInterval = pingInterval;
        // but it does own this "stallDb", lol
        this.stalls = this.findStalls();
        this.fresh = true;
    }
    ...
    newTick() {
         ...
         */
        if (this.activePing === undefined) {
            log.error("tick without ticking. quite the dilemma");
            return;
        }
        this.pingStalls(this.db);
        this.currentTick += 1;
    ...
    findStalls() {
        ...
        for (let stall of map.rides.filter((gayRide) => gayRide.classification === "stall" && gayRide.status === "open")) {
    ...
    pingStalls(db: GuestDb) {
        ...
        for (let [stall, tileCoords] of this.stalls) {
            ...
            // find guests in front of the stall and tell them to come here
            const coords = {
                x: tileCoords.x,
                y: tileCoords.y,
            };
            ...
            const guests = getGuestsOnNeighbouringTile(tileCoords);
            log.verbose(`found ${guests.length} guests next to ${stall.id}`);
            for (const guest of guests) {
                if (!isValidGuest || guest.happiness < 41) continue;
                ...
                favouriteGender; //stop yelping, linter
                ...
                // make guest face stall
                guest.direction = (tileCoords.direction + (tileCoords.direction < 2 ? +2 : -2)) as Direction;
